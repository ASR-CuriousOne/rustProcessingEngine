use rust_processing_engine::{BenchConfig, Customer, process_csv_parallel_callback};
use std::env;
use std::fs;
use std::process;
...
fn main() {
    ...
    println!(
        "Starting PARALLEL benchmark on directory: {:?}",
        config.dir_path
    );
    ...
    for entry in entries {
        ...
        if path.is_file() && path.extension().and_then(|s| s.to_str()) == Some("csv") {
            ...
            let file_rows = AtomicU64::new(0);
            let file_start = Instant::now();

            let result =
                process_csv_parallel_callback(file, batch_size, num_threads, |_customer: Customer| {
                    file_rows.fetch_add(1, Ordering::Relaxed);
                });

            let file_elapsed = file_start.elapsed();

            let final_file_rows = file_rows.load(Ordering::Relaxed);
